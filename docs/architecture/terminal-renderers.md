...
The platform adapter deliberately owns only platform behavior: browser font shaping, the hidden IME
textarea, clipboard and DOM input,
...
To change the upstream pin, update `native/libghostty-vt/VERSION` (the upstream `LICENSE` lives
beside it). Then run:
...
checks it against the vendored `VERSION` — so the web vendor directory holds only the artifacts, drift
