...
Use this when you want to connect to a OpenBuff server from another device such as a phone, tablet, or separate desktop app.
...
run a headless server from the CLI, or have the desktop app launch OpenBuff over SSH.
...
Use this when you want the desktop app to start or reuse OpenBuff on another machine over SSH.
...
The remote host must have a compatible Node.js runtime. OpenBuff uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, OpenBuff first checks whether `node` is on `PATH`. If it is missing, the launcher
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path OpenBuff uses:
...
When the OpenBuff web or desktop app and a remote server use different versions, a warning appears in
