...
This guide is for people who want to use more than one Codex account in OpenBuff. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install OpenBuff](./install.md).
...
In OpenBuff Settings, name it something obvious:
...
In OpenBuff Settings, add another Codex provider:
...
OpenBuff shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, OpenBuff treats it
