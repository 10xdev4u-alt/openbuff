...
This guide is for people who want to use more than one Claude setup in OpenBuff. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install OpenBuff](./install.md).
...
In OpenBuff Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means OpenBuff uses Claude Code's normal config directory.

When you set this field, OpenBuff points Claude Code at that directory with the
...
OpenBuff looks for Claude skills in the Claude config directory's `skills` folder, then
...
`~/.claude_personal_home/.claude`, which is not where OpenBuff looks.

Then add another Claude provider in OpenBuff:
...
OpenBuff only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so OpenBuff keeps separate config directories isolated
...
Add or edit a Claude provider in OpenBuff Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. OpenBuff stores the value as a server secret and does not
...
OpenBuff does not need a special Claude Code Router provider. Treat the router as a Claude
