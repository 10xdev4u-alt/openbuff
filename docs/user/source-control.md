...
OpenBuff connects to your Git hosting provider so you can create pull requests, review code, and manage repositories without leaving the app.

## Supported Providers

OpenBuff works with the platforms your team already uses:
...
- OpenBuff can suggest titles and descriptions based on your commits
...
- Command-click (Control-click on Windows and Linux) a pull request number in the sidebar to open it in your browser instead of in OpenBuff
...
1. Install the GitHub CLI on the machine running OpenBuff:
   ```bash
   brew install gh
   ```
2. Sign in:
   ...
   gh auth login
   ```
3. Open **Settings → Source Control** in OpenBuff and verify GitHub shows as authenticated
...
If both are set, the access token wins. Restart OpenBuff and verify the connection in **Source
Control settings**.
...
**Git is required** – OpenBuff uses Git for all local operations. Ensure `git` is installed on your server.

**Server-side setup** – Authentication happens on the machine running OpenBuff (the server), not your local browser. If you're using a hosted or team instance, your administrator may have already configured providers.
