...
- Web dev bind (post-#29): unconfigured binds `::` (dual-stack, v4-mapped) via `apps/web/src/devBindHost.ts` (probe-`::`-else-`0.0.0.0`; explicit `HOST` wins verbatim — desktop `127.0.0.1`). Cause of the earlier `[::1]:5733`-only binding: Node resolves a *hostname* (`localhost`) to its first address only, OS-order dependent.
